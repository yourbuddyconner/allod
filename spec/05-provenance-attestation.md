# Part 5 — Provenance & Attestation *(L1 lineage / L3 attestation)*
...
answer, from the log alone: "show me every document this claim rests on."
...
- The envelope is agnostic to evidence format. A new TEE platform adds an
  `evidence_type` entry. It does not require a spec revision.
...
- An envelope with `evidence: none` is just a signature. It attests to key
  possession, not execution integrity. Verification results MUST
  distinguish the two.
...
public keys. Level 4 also needs hardware vendor roots.
...
4. **Attestation** (L3). Every envelope's evidence chain verifies to a
   trusted measurement root. The measured code identity is one the
   verifier accepts.

Verification MUST NOT require contact with the graph's host. This is
design principle 3 in operational form.
...
- **Subgraph proofs.** The state hash is a Merkle root (§1.7). A holder
  can reveal a subgraph plus Merkle paths. This proves membership in a
  state without revealing siblings.
- **Attested predicates.** At L3, an attested environment can evaluate a
  predicate over a private graph. It emits an envelope that attests the
  result. Example: "a node classified `kyc/verified` exists for subject
  S." The fact is revealed. The node is not. The predicate's code
...
- **Redacted projections.** A redacted projection (Part 7) carries
  tombstone hashes. Its state stays checkable against the full graph's
  state hash.

## 5.5 Attested execution (L3)

Two workloads justify TEE execution. Both are small on purpose.
...
1. **The attested indexer** (Part 8). Classification runs inside an
   ...
   hash. Consumers can trust machine-written knowledge to exactly the
   degree they trust the measured pipeline. This matters most in
   multi-writer and federated settings, where writers do not trust each
   other.
2. **The attested admission gate** (§4.5). The L2-enforced write path
   runs attested. A third party can verify that nothing enters the graph
   except through policy. "Our process requires review" changes from an
   org-chart claim into a cryptographic one. Compliance regimes buy that
   property.

L3 is an optional strengthening. Nothing outside this section depends on
it (design principle 2). The reference MVP demonstrates it once. It does
not require it throughout.
