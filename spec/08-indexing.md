...
| Basis | Definition | Basis for trust |
|---|---|---|
...
The import has two layers that must never be conflated:
...
2. **The derived graph (this section).** An indexer walks the repo at a
   ...
   and edges for imports, calls, references, definitions, and
...
- **Edge sources.** Deterministic extraction SHOULD use existing
  ...
  preferred and LSIF is acceptable. Allod adds governance, lineage,
...
- **Region reach is file-granular, evaluated at the parent state.** A region rule
  ...
  diff below is material for reviewers, not the matching mechanism.
...
the codebase: which functions changed, what calls into them, and the
blast radius. The graph computes this result instead of a reviewer
estimating it. A review artifact (§4.4) can anchor its sections to both
the file hunks and the affected subgraph, so a reviewer, human or agent,
gets the full-context, both-versions view that diff-only tooling cannot
provide.
...
- A schema version bump or an indexer upgrade MAY trigger re-indexing.
  ...
  lineage, so you can always ask what rust-analyzer 0.3 thought this
  was.
...
- When an input document is redacted (§3.2.2), derived objects that cite
  ...
  resting on removed evidence, and is never silently orphaned.
