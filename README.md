...
The central design decision: **Allod is a changeset format, not a file
...
wire messages are projections of that log. They are never the source of
truth.

Governance, provenance, and attestation are defined over an abstract
changeset substrate. They apply equally to Allod's native changesets and to
git repositories. Git is treated as a second, already-deployed substrate.
...
| 2 — Schema: ontology + taxonomy | [spec/02-schema.md](spec/02-schema.md) | L0 |
...
| 5 — Provenance & attestation | [spec/05-provenance-attestation.md](spec/05-provenance-attestation.md) | L1/L3 |
| 6 — Principals & identity | [spec/06-principals.md](spec/06-principals.md) | L1 |
...
| Appendices A–F | [spec/appendices.md](spec/appendices.md) | — |
...
2. The objects are typed knowledge: entities, relationships,
   classifications. They are not files.
...
4. Classification and admission can run inside attested hardware. A third
   party can then verify two things: what the graph says, and that the
   graph came to say it lawfully.
