...
The packages are validated by [allod-lint](crates/allod-lint/), built
on [allod-core](crates/allod-core/), the shared domain crate holding
the spec vocabulary, package and taxonomy registries, reference
resolution, the attribute type grammar, the canonical wire encoding
(§7.1), and the hashing layer (§1.7, §3.2.6). The linter checks
ontology, taxonomy, policy, and example files against the rules of
Parts 1, 2, and 4, and verifies every import hash. Run it with
`cargo run --release -p allod-lint`.

The Appendix H test vectors in [spec/vectors/](spec/vectors/) are
generated by [allod-vectors](crates/allod-vectors/) and held
reproducible by CI: canonical preimages, changeset and state hashes,
an elision proof, an intent redaction, and the package content hashes
the import declarations bind to.
